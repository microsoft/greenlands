...
public class TasksService : ITasksService
{
    ...
    public async Task<IList<PlaiGroundTask>> Get(string tournamentId, TaskQueryOptions? options = null)
    {
        List<PlaiGroundTask> tasks;
        ...
        else
        {
            ...
            tasks = await CosmosUtilities.ReadAll<PlaiGroundTask>(_tasksCosmosContainer, query, _logger);
        }

        return tasks;
    }

    public async Task<IList<PlaiGroundTask>> FindManyByIds(ISet<string> taskIds)
    {
        var query = new QueryDefinition($"SELECT * FROM {ContainerName} t WHERE ARRAY_CONTAINS(@taskIds, t.id)")
            .WithParameter("@taskIds", taskIds);
        var tasks = await CosmosUtilities.ReadAll<PlaiGroundTask>(_tasksCosmosContainer, query, _logger);

        return tasks;
    }

    public async Task<PlaiGroundTask?> Find(string tournamentId, string taskId, TaskQueryOptions? options)
    {
        var task = await CosmosUtilities.FindItem<PlaiGroundTask>(_tasksCosmosContainer, taskId, new PartitionKey(tournamentId));
        if (task == null)
        {
            return default;
        }
    ...
    public async Task<PlaiGroundTask> Add(string tournamentId, TaskInput taskInput)
    {
        ...
        var task = new PlaiGroundTask(tournament.Id, taskInput)
        {
    ...
    public async Task<PlaiGroundTask> Update(PlaiGroundTask task)
    {
    ...
    public async Task<PlaiGroundTask> Delete(string tournamentId, string taskId)
    {
        ...
        var task = await _tasksCosmosContainer.DeleteItemAsync<PlaiGroundTask>(taskId, new PartitionKey(tournamentId));

        return task;
    }

    public async Task<GameState> GetInitialGameState(PlaiGroundTask task)
    {
    ...
    public async Task<IList<GameChanges>> GetTargetGameChanges(PlaiGroundTask task)
    {
