...
USAGE:
    create-docker-image.sh BUILD_CONTEXT_PATH="context-path" POETRY_REPO_KEY="your-key" WANDB_TOKEN="your-key" IMAGE_NAME="plaiground/mhb-agent" IMAGE_TAG="0.0.1-GPU"
...
ARGS:
    ...
    [required] POETRY_REPO_KEY - ADO Token used to authenticate with PlaiGround private artifact repository
    [required] WANDB_TOKEN - W&B token used to download the weights (can be obtained from https://wandb.ai/authorize)

    [optional - Default: "plaiground/mhb-agent"] IMAGE_NAME - Name of the resulting Docker image
...
[ -z "$IMAGE_NAME" ] && IMAGE_NAME="plaiground/mhb-agent"
